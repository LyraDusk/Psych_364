...
At first we considered using multilevel modeling for our analysis since we included same sex male couples and wanted to account for the nonindependence( proportion of variance explained by the
dyad) effect on our signifcance tests. However, since there weren't enough dyads in our dataset we used a multiple regression model instead. We hypothesized that higher childcare tasks would negatively affect father's relationship satisfaction. The gatekeeping of a couple usually tends to affect one another to the point where it can not be considered independent. To measure how our dyads influence each other, we used the Actor-Partner Interdependence Model [@Kenny]. We used the APIM and mutliple regression model to test the interaction between childcare, actor gatekeeping and partner's high level of gatekeeping. The APIM allows us to assess how the fathers' and their partner's gatekeeping affect their childcare and relationship satisfaction. To arrive at our final model we used a mutiple regression model including father's childcare, actor and partner gatekeeping, and childcare interaction.
...
Contrary to our original hypothesis, father's childcare tasks do not significantly affect their relatioship satisfaction when their gateekping is at the mean and their partner's gatekeeping is high, ($b$ = -0.01, $SE$ $\leq$ 0.01, $p$ = 0.27).  
When father's childcare tasks is at 0, their own and their partner's gatekeeping do not have a significant effect on their relationship satisfaction, ($b$ = -0.56, $SE$ = 0.32, $p$ = 0.08), ($b$ = -0.37, $SE$ = 0.30, $p$ = 0.23). The negative effect of partner's gatekeeping on the father's relationship satisfaction is not significantly affected when they do more childcare tasks, ($b$  $\leq$ 0.01, $SE$  $\leq$ 0.01, $p$ = 0.31). Similarly, childcare tasks do not significantly affect the father's gatekeeping effect on their relationship satisfaction, ($b$  $\leq$ 0.01, $SE$  $\leq$ 0.01, $p$ = 0.27)
